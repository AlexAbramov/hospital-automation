...
//		public int Age { get { return birthday != DateTime.MinValue ? DateTimeUtils.Age(birthday) : -1; } }
		...
		public PatientIdentification(DbDataReader dr)
		{
			...
			int i = 0;
			id = gr.GetInt32(i++);
			surname = gr.GetString(i++);
			name = gr.GetString(i++);
			middleName = gr.GetString(i++);
			gender = (GenderId)gr.GetInt(i++);
			birthday = gr.GetDateTime(i++);
			identificationData = IdentificationData.Create(gr.GetString(i++));
		}
		#endregion
